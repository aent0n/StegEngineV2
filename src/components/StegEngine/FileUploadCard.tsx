// Résumé du fichier : Composant pour gérer les téléchargements de fichiers et afficher les informations de fichier
// pour divers outils de stéganographie. Inclut l'affichage de la capacité et la saisie de message pour l'intégration.
"use client";
...
const FileIconDisplay = ({ fileType }: { fileType: string | null }) => {
  ...
  if (fileType === "application/pdf") return <FileTextIcon className="w-16 h-16 text-muted-foreground" />; // Specific check for PDF
  if (fileType.startsWith("text/")) return <FileTextIcon className="w-16 h-16 text-muted-foreground" />;
